...
Background
----------
A NAS-hosted Fly agent's Nous bootstrap session (client_id ``hermes-cli-vps``)
can take a terminal ``invalid_grant`` and be quarantined locally — the refresh
path clears the dead tokens from ``auth.json`` and stamps
``providers.nous.last_auth_error.relogin_required = true``. From then on every
inference turn hard-fails with a provider-auth error until a human re-logs-in,
even though the machine, relay, and dashboard all look healthy.
...
plain restart with a fresh seed env can NOT recover an existing box.

This script is the narrow, safe exception. NAS re-mints a fresh bootstrap
session server-side and pushes it as ``HERMES_AUTH_JSON_REBOOTSTRAP`` (plus a
restart). On boot we re-seed the Nous provider entry from that env **only when
the on-disk Nous entry is provably terminal** (the quarantine marker above with
no usable tokens left). Every other case is a no-op, so we never clobber a
healthy or merely-rotating session.
...
- Fail-safe: any parse/IO error leaves auth.json exactly as-is and exits 0 (a
  failed re-seed must never take the box further down than it already is).
"""
...
# Env var NAS sets to the re-seed payload. Deliberately DISTINCT from
# HERMES_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two paths
# can never be confused: BOOTSTRAP seeds a fresh volume; REBOOTSTRAP overwrites
# a terminally-dead Nous entry on an existing volume.
...
def _nous_entry_is_terminal(nous_state: Any) -> bool:
    ...
    exactly the boxes NAS observed as ``nous_session_valid == "terminal"``.
    """
...
def _extract_nous_from_seed(seed_raw: str) -> Optional[dict]:
    ...
    payload. The payload is a full auth.json document (same shape NAS builds for
